...
function _hrb_project_map_capabilities( $caps, $cap, $user_id, $args ) {
	...
	switch( $cap ) {
		...
		case 'edit_comment':
			...
			// only allow comment edits for the comment author or admins
			if ( ! is_super_admin() && HRB_PROJECTS_PTYPE == get_post_type( $post ) && $comment->user_id != $user_id ) {
				$caps[] = 'do_not_allow';
			}
			break;
...
function _hrb_bid_map_capabilities( $caps, $cap, $user_id, $args ) {
	...
	switch( $cap ) {
		...
		case 'edit_bid':
			...
			// @todo: maybe remove this cap check in the future -
			// meaning allowing a user that participated on a canceled project to edit his original bid and be selected again
			// make sure the workflow is consistent

			// Don't allow if user is/was a participant on any of the project workspaces.
			$workspaces_ids = hrb_get_participants_workspace_for( $post->ID, $user_id );

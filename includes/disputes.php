...
function _hrb_project_status_change_inform_dispute( $content, $new_status, $post, $workspace_id, $user ) {
	...
	if ( $post->post_author == $user->ID ) {
		...
		$content .= sprintf( __( 'If you do not agree with the decision, you can open a dispute in the project %1$s within the next %2$d %3$s.', APP_TD ), $workspace_link, $days_left, _n( 'day', 'days', $days_left, APP_TD ) );
	}

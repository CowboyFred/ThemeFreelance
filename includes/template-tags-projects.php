...
function hrb_output_project_actions( $post_id ) {
	...
	// view workspace
	the_hrb_project_workspace_link( $post_id );
}
...
function the_hrb_project_edit_link( $post_id = 0, $text = '', $before = '', $after = '' ) {
	...
	echo html( 'a', array(
		...
		'href' => esc_url( get_the_hrb_project_edit_url( $post_id ) ),
	), $before . $text . $after );
}
...
function the_hrb_proposals_view_link( $post_id = 0, $text = '', $before = '', $after = '', $min = 1, $atts = array() ) {
	...
	$default = array(
		...
		'href' => esc_url( get_the_hrb_project_proposals_url( $post_id ) ),
	);
...
function get_the_hrb_project_favorite_link( $post_id = 0, $before = '', $after = '', $atts = array() ) {
	...
	if ( ! is_hrb_faved_project( $post_id ) ) {
		...
		$defaults = array(
			...
			'href'      => esc_url( get_the_hrb_favorite_project_url( $post_id, 'add', $atts['base_url'] ) ),
		);
		$atts = wp_parse_args( $atts, $defaults );
		...
		$defaults = array(
			'unfave_text' => __( 'Unfavorite', APP_TD ),
			'class'       => "button secondary expand {$post_type}-unfave-link",
			'href'        => esc_url( get_the_hrb_favorite_project_url( $post_id, 'delete', $atts['base_url'] ) ),
		);
		$atts = wp_parse_args( $atts, $defaults );
...
function the_hrb_project_excerpt( $before = '', $after = '' ) {
	...
	echo $before . ob_get_clean() . $after;
}
...
function the_hrb_project_workspace_link( $post_id = 0, $text = '', $before = '', $after = '' ) {
	...
	if ( count( $workspaces ) > 1  ) {
		the_hrb_data_dropdown( $workspaces, array( 'data-dropdown' => "workspaces-{$post_id}" ), $before .  __( 'Workspaces', APP_TD ) . $after );
	} else {
		the_hrb_workspace_link( $workspaces_ids[0], $text, $before, $after, array( 'class' => 'button secondary expand' ) );
	}
}
...
function is_hrb_project_saveable_filter() {
	return ! is_hrb_users_archive() && ( is_post_type_archive( HRB_PROJECTS_PTYPE ) || is_search() || is_tax( HRB_PROJECTS_CATEGORY ) || is_tax( HRB_PROJECTS_SKILLS ) );
}

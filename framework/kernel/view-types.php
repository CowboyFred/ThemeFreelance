...
class APP_View_Page extends APP_View {
	 ...
	 *
	 * @todo Use filter "theme_{$post_type}_templates" to add extra templates.
	 * It was added since WP 3.9, we support WP since 4.5.6, so we can use it.
	  */
	public static function _register_new_templates ( $atts = array() ) {
		global $wp_version;

		if ( version_compare( $wp_version, '4.6.4', '>' ) ) {
			$cache_key = 'post_templates-';
		} else {
		...
		if ( version_compare( $wp_version, '4.6.4', '>' ) ) {
			$templates = array( 'page' => $templates );
		}
